      ...
      {showingLabel && (
        <div className="attached-banner absolute left-6 right-6 top-2 z-0 rounded-t-2xl border border-[#f3f3f3] bg-white px-4 py-2 shadow-[0_4px_6px_-1px_rgba(229,229,229,0.8),0_2px_4px_-2px_rgba(229,229,229,0.8)]">
          <button className="flex w-full items-center justify-between gap-3 text-left" onClick={onClearShowing}>
            <span className="truncate text-sm leading-5 tracking-[0.015em] text-neutral-950">Showing {showingLabel}</span>
            <span className="shrink-0 text-sm font-semibold leading-5 tracking-[0.015em] text-red-600">Remove</span>
          </button>
        </div>
      )}
      <div className="relative z-10 space-y-1.5 rounded-lg bg-white">
        {items.map((item) => {
          const visualMax = Math.max(item.range.max * 1.22, item.value * 1.08, 1);
          const fillWidth = Math.min(100, Math.max(3, (item.value / visualMax) * 100));
          return (
